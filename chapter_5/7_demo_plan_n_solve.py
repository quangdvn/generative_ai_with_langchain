...
def calculator(state: CalculatorArgs, config: RunnableConfig) -> str:
	...
	result = operator.ne.evaluate(expression.strip(), local_dict=math_constants)
	return str(result)
...
async def _run_step(state: PlanState) -> PlanState:
	...
	print("=" * 50)
	print("Current full plan: ", get_full_plan(state))
	print("=" * 50)
	...
	# print("Final step: ", step["messages"][-1].content)
